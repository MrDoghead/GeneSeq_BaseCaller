...
from bonito.utils.util import __data__, __models__
...
def main(args):
    ...
    if args.models or args.all:
        ...
        if args.show:
            ...
            print("[downloading models to {s}]".format(s=args.model_dir))
            for model in models.values():
                File(args.model_dir, model, args.force).download()
    if args.training or args.all:
        print("[downloading training data to {s}]".format(s=args.data_dir))
        for train in training:
            File(args.data_dir, train, args.force).download()
...
def argparser():
    ...
    parser.add_argument('--model_dir', type=str, default='./model_bin/ont_models')
